...
//   title: "Sign Up | Univent - University Event Platform",
//   description: "Create your Univent account to discover or host campus events",
// };
...
export default function Signup() {
  ...
  const validate = () => {
    ...
    if (formData.accountType === 'college' && !formData.collegeName.trim()) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      router.push(formData.accountType === 'student' ? '/dashboard' : '/college/dashboard');
    } catch (err) {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={loading}
                  className={`w-full flex justify-center py-3 px-4 border border-transparent rounded-full shadow-sm text-sm font-medium text-white bg-orange-500 hover:bg-orange-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-colors ${loading ? 'opacity-70 cursor-not-allowed' : ''}`}
                >
                  {loading ? (
                    <>
